  ...
  try {
    ...
    await connection.query(`
      CREATE TABLE IF NOT EXISTS quizzes (
        ...
        snapshot_interval_min INT DEFAULT 60,
        snapshot_interval_max INT DEFAULT 180,
        max_focus_violations INT DEFAULT 3,
        created_by INT NOT NULL,
        created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    ...
    // Ensure new proctoring columns exist for legacy databases
    const proctorColumns = [
      { name: 'enable_proctoring', definition: 'ADD COLUMN enable_proctoring BOOLEAN DEFAULT FALSE' },
      { name: 'snapshot_interval_min', definition: 'ADD COLUMN snapshot_interval_min INT DEFAULT 60' },
      { name: 'snapshot_interval_max', definition: 'ADD COLUMN snapshot_interval_max INT DEFAULT 180' },
      { name: 'max_focus_violations', definition: 'ADD COLUMN max_focus_violations INT DEFAULT 3' }
    ];

    for (const column of proctorColumns) {
      try {
        await connection.query(`ALTER TABLE quizzes ${column.definition}`);
      } catch (error) {
        if (!error.message.includes('Duplicate column name')) {
          console.warn(`Warning adding column ${column.name} to quizzes:`, error.message);
        }
      }
    }
    ...
    await connection.query(`
      CREATE TABLE IF NOT EXISTS proctor_events (
        ...
        event_type ENUM('focus_lost', 'focus_regained', 'snapshot', 'warning') NOT NULL,
        details JSON,
        created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
