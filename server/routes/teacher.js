  ...
  try {
    ...
    try {
      const [quizResult] = await connection.query(
        `INSERT INTO quizzes (
          experiment_id, subject_id, title, total_marks, duration_minutes, 
          start_date, end_date, enable_proctoring, snapshot_interval_min, 
          snapshot_interval_max, max_focus_violations, created_by
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        [
          experiment_id,
          subject_id,
          ...
          start_date,
          end_date,
          proctoring?.enable ? 1 : 0,
          proctoring?.snapshot_interval_min ? parseInt(proctoring.snapshot_interval_min, 10) : 60,
          proctoring?.snapshot_interval_max ? parseInt(proctoring.snapshot_interval_max, 10) : 180,
          proctoring?.max_focus_violations ? parseInt(proctoring.max_focus_violations, 10) : 3,
          req.user.id
        ]
      );
...
router.put('/quizzes/:id', async (req, res) => {
  try {
    const { title, total_marks, duration_minutes, start_date, end_date, is_active, proctoring } = req.body;
    ...
    if (proctoring) {
      if (proctoring.enable !== undefined) {
        updates.push('enable_proctoring = ?');
        values.push(proctoring.enable ? 1 : 0);
      }
      if (proctoring.snapshot_interval_min !== undefined) {
        updates.push('snapshot_interval_min = ?');
        values.push(parseInt(proctoring.snapshot_interval_min, 10));
      }
      if (proctoring.snapshot_interval_max !== undefined) {
        updates.push('snapshot_interval_max = ?');
        values.push(parseInt(proctoring.snapshot_interval_max, 10));
      }
      if (proctoring.max_focus_violations !== undefined) {
        updates.push('max_focus_violations = ?');
        values.push(parseInt(proctoring.max_focus_violations, 10));
      }
    }
