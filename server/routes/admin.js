...
// Helper to resolve student identifier (user_id or numeric id) to internal id
const resolveStudentId = async (studentIdentifier) => {
  if (!studentIdentifier) return null;
  const [rows] = await db.pool.query(
    'SELECT id FROM users WHERE user_id = ? OR id = ? LIMIT 1',
...
router.get('/stats/student-scores', async (req, res) => {
  try {
    ...
    const params = [];
    if (student_id) {
      const resolvedId = await resolveStudentId(student_id);
      if (!resolvedId) {
        return res.json([]); // No matching student, return empty result
      }
      query += ' AND u.id = ?';
